...
- **Read interval** - интервал чтения сенсора. От 5 до 600 секунд с шагом 1. Этому параметру стоит уделить отдельное внимание. Этот параметр также влияет на то время, которое модуль "спит", т.е. находится в режиме экономии батареи. По большому счету модуль "спит" весь этот период. По умолчанию этот период задан в 10 секунд. Модуль просыпается каждые 10 секунд, считывает данные из сенсора (температуру и влажность) и засыпает опять на 10 секунд. Но нужно учитывать этот параметр также завязан на настройку репортинга. Поэтому время сна выбирается исходя из меньшего значения между этим параметром и настройкой репортинга. Например, если этот параметр установлен в 20 секунд, а `MinIntrerval` для температуры или влажности будет установлен в 15 секунд, модуль будет спать 15 секунд. И считывание сенсора будет происходить тоже раз в 15 секунд. Т.е. выбирается наименьшее значение между `Read interval` и настройкой репортинга.
